            ...
            switch (newState) {
                    ...
                    switch (oldState) {
                        case RECEIVING_HEADER: {
                            ...
                            request.completionHandler
                                    .failed(new IOException("Timeout receiving response", connection.getError()));
                            return;
                        }
                        ...
                        case RECEIVING_BODY: {
                            ...
                            request.response.getBodyStream()
                                    .notifyError(new IOException("Timeout receiving response body", connection.getError()));
                            return;
                        }
                ...
                case CLOSED_BY_SERVER: {
                    switch (oldState) {
                        case SENDING_REQUEST: {
                            RequestRecord request = removeRequest(connection);
                            request.completionHandler.failed(new IOException("Connection closed by server while sending request",
                                    connection.getError()));
                            return;
                        }
                        ...
                        case RECEIVING_HEADER: {
                            RequestRecord request = removeRequest(connection);
                            request.completionHandler
                                    .failed(new IOException("Connection closed by server while receiving response",
                                            connection.getError()));
                            return;
                        }
                        ...
                        case RECEIVING_BODY: {
                            ...
                            request.response.getBodyStream().notifyError(
                                    new IOException("Connection closed by server while receiving response body",
                                            connection.getError()));
                            return;
                        }

                        case CONNECTING: {
                            removeAllPendingWithError(new IOException("Connection closed by server"));
                            return;
                        }
                    }
                }
                ...
                case CLOSED: {
                    switch (oldState) {
                        case SENDING_REQUEST: {
                            RequestRecord request = removeRequest(connection);
                            request.completionHandler.failed(new IOException("Connection closed by client while sending request",
                                    connection.getError()));
                        ...
                        case RECEIVING_HEADER: {
                            RequestRecord request = removeRequest(connection);
                            request.completionHandler
                                    .failed(new IOException("Connection closed by client while receiving response",
                                            connection.getError()));
                            cleanClosedConnection(connection);
                            return;
                        }
                        ...
                        case RECEIVING_BODY: {
                            RequestRecord request = requestsInProgress.remove(connection);
                            request.response.getBodyStream().notifyError(
                                    new IOException("Connection closed by client while receiving response body",
                                            connection.getError()));
                            cleanClosedConnection(connection);
                            return;
                        }
                ...
                case CONNECT_TIMEOUT: {
                    switch (oldState) {
                        case CONNECTING: {
                            removeAllPendingWithError(new IOException("Connection timed out"));
                            return;
                        }
