...
class HttpConnection {
    ...
    private synchronized void handleHeaderSent() {
        ...
        if (httpRequest.getBodyMode() == HttpRequest.BodyMode.NONE
            ...
            bodyStream.setCloseListener(new ChunkedBodyOutputStream.Listener() {
                @Override
    ...
    private void processResponseHeaders(HttpResponse response) throws IOException {
        ...
        if (connectionValues != null) {
            for (String connectionValue : connectionValues) {
                if (connectionValue.equalsIgnoreCase(Constants.CONNECTION_CLOSE)) {
    ...
    private void changeState(State newState) {
        ...
        if (LOGGER.isLoggable(Level.FINEST)) {
            LOGGER.finest(
                    "HTTP connection " + uri.getHost() + ":" + uri.getPort() + " changing state " + old + " -> " + newState);
        }
