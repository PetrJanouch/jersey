...
class ProxyFilter extends Filter<HttpRequest, HttpResponse, HttpRequest, HttpResponse> {
        ...
        switch (httpResponse.getStatusCode()) {
            ...
            case 407: {
                if (state == State.AUTHENTICATED) {
                    upstreamFilter.onError(new ProxyAuthenticationException("Received 407 for the second time."));
                    return false;
                }
            ...
            default: {
                handleError(new IOException("Connecting to proxy failed with status " + httpResponse.getStatusCode()));
            }
        }
    ...
    private HttpRequest createAuthenticatingRequest(HttpResponse httpResponse) throws ProxyAuthenticationException {
        ...
        if (authenticateHeader == null || authenticateHeader.equals("")) {
            throw new ProxyAuthenticationException(Constants.PROXY_AUTHENTICATE + " header value is mussing or empty");
        }
        ...
        if (Constants.BASIC.equals(scheme)) {
            ...
            throw new ProxyAuthenticationException("Unsupported scheme: " + scheme);
        }
